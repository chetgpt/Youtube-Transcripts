...
def download_videos_from_playlist(playlist_url, download_path):
    """Download all videos from a YouTube playlist and return video paths along with YouTube objects, skipping age-restricted videos."""
...
def download_video_with_retry(yt, download_path, max_retries=3):
    """Attempt to download a video with retries on failure."""
    retry_count = 0
    while retry_count < max_retries:
        try:
            video = yt.streams.get_highest_resolution()
            video_path = video.download(download_path)
            return video_path  # Return on successful download
        except Exception as e:
            print(f"Failed to download video {yt.watch_url}: {e}. Retrying...")
            time.sleep(5)  # Wait for 5 seconds before retrying
            retry_count += 1
    print(f"Failed to download video {yt.watch_url} after {max_retries} retries.")
    return None
